...
def run_and_save_experiment(experiment_name, initialized_env, experiment_kwargs=None, agent_kwargs=None,
    ...
    experiment_params = {k: v.default for k, v in inspect.signature(dqn).parameters.items()
                         if v.default is not inspect.Parameter.empty}
    experiment_params.update(experiment_kwargs)

    # combine default agent kwargs with user-provided overrides
    agent_params = {k: v.default for k, v in inspect.signature(Agent).parameters.items()
                    if v.default is not inspect.Parameter.empty and k != "env"}
