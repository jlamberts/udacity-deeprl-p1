...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed=None, double_dqn=False, network=QNetwork, network_kwargs=None,
                buffer_size=int(1e5), batch_size=64, gamma=.99, tau=1e-3, lr=5e-4, update_every=4):
        ...
        if seed:
            self.qnetwork_local = network(state_size=state_size, 
                                          action_size=action_size, seed=seed, **network_kwargs).to(device)
            self.qnetwork_target = network(state_size=state_size, 
                                           action_size=action_size, seed=seed, **network_kwargs).to(device)
        else:
            self.qnetwork_local = network(state_size=state_size, 
                                          action_size=action_size, **network_kwargs).to(device)
            self.qnetwork_target = network(state_size=state_size, 
                                           action_size=action_size, **network_kwargs).to(device)
